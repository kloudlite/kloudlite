...
- Use the `coordinator` agent for project work; specialist engineering agents are not configured in this project.
- Start feature work from updated `development` and create a focused task branch unless the user explicitly chooses another branch/workflow.
- Keep edits scoped to the user's request and the affected areas.
