...
func (r *EnvironmentSnapshotRequestReconciler) Reconcile(ctx context.Context, req reconcile.Request) (reconcile.Result, error) {
	...
	if err := r.Get(ctx, client.ObjectKey{Namespace: envSnapshotReq.Spec.EnvironmentNamespace, Name: envSnapshotReq.Spec.EnvironmentName}, env); err != nil {
		if apierrors.IsNotFound(err) {
...
func (r *EnvironmentSnapshotRequestReconciler) createSnapshotArtifacts(ctx context.Context, snapshotName, namespace string, env *environmentsv1.Environment, logger *zap.Logger) error {
	...
	// Capture full Environment spec for forking
	if env != nil {
		// Create a clean copy of the spec for storage
		// Clear fields that should be regenerated when forking
		envSpecCopy := env.Spec.DeepCopy()
		envSpecCopy.TargetNamespace = "" // Will be auto-generated by webhook
		envSpecCopy.FromSnapshot = nil   // Not needed in stored spec
		envSpecCopy.WorkMachineName = "" // Will be derived from namespace
		envSpecCopy.NodeName = ""        // Will be derived from workmachine

		envSpecData, err := json.Marshal(envSpecCopy)
		if err != nil {
			logger.Warn("Failed to marshal environment spec", zap.Error(err))
		} else {
			artifacts.Spec.EnvironmentSpec = base64.StdEncoding.EncodeToString(envSpecData)
			logger.Info("Captured full environment spec")
		}

		// Also capture ComposeSpec for backward compatibility
		if env.Spec.Compose != nil {
			...
			if err != nil {
				...
				logger.Info("Captured environment compose spec (backward compat)")
			}
		}
	}
