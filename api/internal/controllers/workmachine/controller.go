...
import (
	...
	ocicloud "github.com/kloudlite/kloudlite/api/internal/controllers/workmachine/cloud/oci"
...
func (r *WorkMachineReconciler) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	return reconciler.ReconcileSteps(req, []reconciler.Step[*v1.WorkMachine]{
		...
		{
			Name:     "legacy/cleanup-code-analyzer",
			Title:    "Cleanup legacy code analyzer resources",
			OnCreate: r.cleanupLegacyCodeAnalyzer,
			OnDelete: r.cleanupLegacyCodeAnalyzer,
		},
		{
