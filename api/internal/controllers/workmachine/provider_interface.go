...
type CloudProviderInterface interface {
	...
	// Security Group Operations - explicit methods for security group management

	// CreateSecurityGroup creates a security group for the WorkMachine
	// Returns the security group ID
	// Returns ResourceAlreadyExistsError if security group already exists
	// CreateSecurityGroup(ctx context.Context, wm *workmachinev1.WorkMachine) (string, error)
	//
	// // GetSecurityGroup retrieves a security group by name
	// // Returns the security group ID or empty string if not found
	// GetSecurityGroup(ctx context.Context, name string) (string, error)
	//
	// // DeleteSecurityGroup deletes a security group by ID
	// // Returns ResourceNotFoundError if security group doesn't exist
