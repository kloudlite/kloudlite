...
import (
	...
	"github.com/kloudlite/kloudlite/api/tunnel/handlers"
	"github.com/kloudlite/kloudlite/api/tunnel/middleware"
	"github.com/kloudlite/kloudlite/pkg/udptunnel/transport"
	"github.com/kloudlite/kloudlite/pkg/udptunnel/tunnel"
...
func Run(ctx context.Context, args []string) error {
	...
	flags.StringVar(&cfg.ListenAddr, "listen", ":443", "Listen address for TLS WebSocket server (e.g., :443)")
	flags.StringVar(&cfg.TLSSecretName, "tls-secret", "tunnel-server-tls", "Kubernetes secret name containing tls.crt and tls.key")
	flags.StringVar(&cfg.WireguardTarget, "wireguard-target", "127.0.0.1:51820", "WireGuard UDP target")
	flags.BoolVar(&cfg.WatchConfig, "watch-config", false, "Watch WireGuard config and reload peers dynamically")
	flags.StringVar(&cfg.ConfigPath, "config-path", "/etc/wireguard/wg0.conf", "Path to WireGuard config file to watch")
	flags.StringVar(&cfg.WgDevice, "wg-device", "wg0", "WireGuard device name")
	flags.StringVar(&cfg.WgCIDR, "wg-cidr", "10.17.0.0/24", "WireGuard CIDR for peer IP allocation")
	flags.StringVar(&cfg.WgServerAddress, "wg-server-address", "10.17.0.1", "WireGuard server address")
	flags.StringVar(&cfg.WgEndpoint, "wg-endpoint", os.Getenv("PUBLIC_HOST"), "WireGuard server public endpoint (e.g., tunnel.example.com:443), can also be set via PUBLIC_HOST env var")
	flags.StringVar(&cfg.CACertSecretName, "ca-cert-secret", "tunnel-server-ca", "Kubernetes secret name containing ca.crt")
	flags.StringVar(&cfg.KltunTLSSecretName, "kltun-tls-secret", "kltun-tls", "Kubernetes secret name containing TLS cert for kltun HTTPS server")
	flags.StringVar(&cfg.Namespace, "namespace", os.Getenv("POD_NAMESPACE"), "Namespace to query for ingresses (defaults to POD_NAMESPACE env var)")
	flags.StringVar(&cfg.RouterServiceRef, "router-service", "wm-ingress-controller", "Name of the router service for hosts resolution")
	flags.StringVar(&cfg.JWTSecret, "jwt-secret", os.Getenv("JWT_SECRET"), "JWT secret for token validation (can also be set via JWT_SECRET env var)")
	flags.StringVar(&cfg.DNSListenAddr, "dns-listen", ":53", "DNS server listen address")
	flags.StringVar(&cfg.UpstreamDNS, "upstream-dns", "10.43.0.10:53", "Upstream DNS server for non-cached queries")
	...
	// No JWT auth needed - if you can reach this endpoint, you're connected to the VPN
	vpnStatusHandler := handlers.NewVPNStatusHandler(logger)
	mux.Handle("/", handlers.WithCORS(vpnStatusHandler))
...
func createK8sClient() (client.Client, *rest.Config, *runtime.Scheme, error) {
	...
	utilruntime.Must(environmentv1.AddToScheme(scheme))
