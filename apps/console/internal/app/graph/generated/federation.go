...
func (ec *executionContext) resolveEntity(
	...
	switch typeName {
	case "Build":
		resolverName, err := entityResolverNameForBuild(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "Build": %w`, err)
		}
		switch resolverName {

		case "findBuildByID":
			id0, err := ec.unmarshalNID2githubᚗcomᚋkloudliteᚋapiᚋpkgᚋreposᚐID(ctx, rep["id"])
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findBuildByID(): %w`, err)
			}
			entity, err := ec.resolvers.Entity().FindBuildByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "Build": %w`, err)
			}
...
func entityResolverNameForBuild(ctx context.Context, rep EntityRepresentation) (string, error) {
	for {
		var (
		...
		return "findBuildByID", nil
	}
	return "", fmt.Errorf("%w for Build", ErrTypeNotFound)
}
