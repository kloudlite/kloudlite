...
|-----|------|---------|-------------|
| accessToken | string | `"‼️ Required"` | kloudlite issued access token (if already have) |
| accountName | string | `"‼️ Required"` | kloudlite account name |
| agent.enabled | bool | `true` | enable/disable kloudlite agent |
| agent.image | string | `"/kloudlite/development/kl-agent:v1.0.5"` | kloudlite agent image name and tag |
...
| operators.resourceWatcher | object | `{"enabled":true,"image":"/kloudlite/operators/development/resource-watcher:v1.0.5","name":"kl-resource-watcher"}` | configuration for different kloudlite operators used in this chart |
| operators.resourceWatcher.enabled | bool | `true` | enable/disable kloudlite resource watcher |
| operators.resourceWatcher.image | string | `"/kloudlite/operators/development/resource-watcher:v1.0.5"` | kloudlite resource watcher image name and tag |
