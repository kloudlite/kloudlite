...
spec:
  adminEndpoint: {{.Values.redpandaCluster.name}}.{{.Release.Namespace}}.svc.{{.Values.clusterInternalDNS}}:9644
  {{- $kafkaBrokers := list }} 
  ...
  {{- end }}
  {{- /* kafkaBrokers: {{.Values.redpandaCluster.name}}-0.{{.Values.redpandaCluster.name}}.{{.Release.Namespace}}.svc.{{.Values.clusterInternalDNS}}:9092 */}}
