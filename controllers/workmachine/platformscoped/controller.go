	...
	if _, err := controllerutil.CreateOrUpdate(ctx, r.DirectClient, statefulSet, func() error {
		...
		statefulSet.Spec.Template.Spec.Containers = []corev1.Container{{
			...
			Ports: []corev1.ContainerPort{
				...
				{Name: "wireguard", ContainerPort: 51820, Protocol: corev1.ProtocolUDP},
			...
			Env: []corev1.EnvVar{
				...
				{Name: "PUBLIC_HOST", Value: obj.Status.PublicIP},
				{Name: "HOSTED_SUBDOMAIN", Value: r.env.HostedSubdomain},
				{Name: "CODE_ANALYZER_IMAGE", Value: r.env.CodeAnalyzerImage},
				{Name: "WORKSPACE_COMPREHENSIVE_IMAGE", Value: r.env.WorkspaceComprehensiveImage},
			...
			VolumeMounts: []corev1.VolumeMount{
				...
				{Name: "tunnel-peer-storage", MountPath: "/var/lib/tunnel-server"},
				{Name: "tunnel-wireguard-config", MountPath: "/etc/wireguard"},
			},
		}}
		statefulSet.Spec.Template.Spec.Volumes = []corev1.Volume{
			...
			hostPathVolume("tunnel-peer-storage", "/var/lib/tunnel-server", corev1.HostPathDirectoryOrCreate),
			{
				Name: "tunnel-wireguard-config",
