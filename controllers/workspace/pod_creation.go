...
`, workspace.Name, workspace.Name, workspace.Name, workspace.Namespace, workspace.Spec.OwnedBy, targetNamespace, imageRegistryURL, searchDomains, hostsEntry, contextJSON)
	...
	if workspace.Spec.ResourceQuota != nil {
		...
		if workspace.Spec.ResourceQuota.CPU != "" {
			limits[corev1.ResourceCPU] = resource.MustParse(workspace.Spec.ResourceQuota.CPU)
		}
		if workspace.Spec.ResourceQuota.Memory != "" {
			limits[corev1.ResourceMemory] = resource.MustParse(workspace.Spec.ResourceQuota.Memory)
		}
