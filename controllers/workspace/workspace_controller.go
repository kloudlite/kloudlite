...
func (r *WorkspaceReconciler) ensureConfig() {
	if r.Cfg != nil {
		cfg = r.Cfg
	}
	if cfg == nil {
	...
	if cfg.Workspace.CleanupPodTTLSeconds == 0 {
		cfg.Workspace.CleanupPodTTLSeconds = 300
	}
	if cfg.Workspace.DefaultIdleTimeoutMinutes == 0 {
		cfg.Workspace.DefaultIdleTimeoutMinutes = 30
	}
	if cfg.Workspace.AlpineImage == "" {
		cfg.Workspace.AlpineImage = "alpine:latest"
	}
	if cfg.Workspace.KubectlImage == "" {
		cfg.Workspace.KubectlImage = "bitnami/kubectl:latest"
	}
...
func shouldRunRBACCleanup() bool {
	if cfg == nil || cfg.Workspace.RBACCleanupIntervalMinutes <= 0 {
		return false
	}
