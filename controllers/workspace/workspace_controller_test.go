	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectError {
				...
				if err != nil && !strings.Contains(err.Error(), "unable to upgrade connection") && !strings.Contains(err.Error(), "connection refused") && !strings.Contains(err.Error(), "unexpected EOF") {
					assert.NoError(t, err)
				}
			}
		})
