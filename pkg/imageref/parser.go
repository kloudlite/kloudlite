...
func Parse(ref string) (*Reference, error) {
	...
	parsedRef, err := name.ParseReference(parseRef, name.StrictValidation)
	if err != nil {
	...
	result := &Reference{
		Full:       ref,
		Registry:   normalizeRegistry(parsedRef.Context().RegistryStr()),
...
func ParseWithDefaults(ref, defaultTag string) (*Reference, error) {
	...
	// Apply default tag if the original reference omitted tag and digest.
	if !hasExplicitTagOrDigest(ref) && defaultTag != "" {
