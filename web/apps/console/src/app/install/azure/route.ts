...
if [ -z "\$INSTALLATION_KEY" ]; then
    ...
    echo "Usage: curl -fsSL https://get.khost.dev/install/azure | bash -s -- --key YOUR_KEY [--location LOCATION] [--resource-group RG]"
    exit 1
fi
...
run_doctor() {
    echo ""
    echo -e "\${BLUE}================================\${NC}"
    echo -e "\${BLUE}  Checking Azure Prerequisites\${NC}"
    echo -e "\${BLUE}================================\${NC}"
    echo ""

    if ! ./kli azure doctor; then
        echo ""
...
run_install() {
    echo ""
    echo -e "\${BLUE}================================\${NC}"
    echo -e "\${BLUE}  Installing Kloudlite on Azure\${NC}"
    echo -e "\${BLUE}================================\${NC}"
    echo ""

    local install_cmd="./kli azure install --installation-key \$INSTALLATION_KEY"

    if [ -n "\$AZURE_LOCATION" ]; then
        install_cmd="\$install_cmd --location \$AZURE_LOCATION"
    fi

    if [ -n "\$AZURE_RESOURCE_GROUP" ]; then
        install_cmd="\$install_cmd --resource-group \$AZURE_RESOURCE_GROUP"
    fi
    ...
    if eval "\$install_cmd"; then
        ...
        if [ -n "\$AZURE_LOCATION" ]; then
            echo -e "\${BLUE}Location: \${AZURE_LOCATION}\${NC}"
        fi
        if [ -n "\$AZURE_RESOURCE_GROUP" ]; then
            echo -e "\${BLUE}Resource Group: \${AZURE_RESOURCE_GROUP}\${NC}"
        fi
    else
        echo ""
...
main() {
    echo -e "\${BLUE}================================\${NC}"
    echo -e "\${BLUE}  Kloudlite Azure Installer\${NC}"
    echo -e "\${BLUE}================================\${NC}"
    echo ""
...
export async function GET(_request: NextRequest) {
  return new NextResponse(INSTALL_SCRIPT, {
    headers: {
      ...
      'Content-Disposition': 'inline; filename="install-azure.sh"',
