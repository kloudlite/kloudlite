...
if [ -z "\$INSTALLATION_KEY" ]; then
    ...
    echo "Usage: curl -fsSL https://get.khost.dev/uninstall/azure | bash -s -- --key YOUR_KEY [--location LOCATION] [--resource-group RG]"
    exit 1
fi
...
run_uninstall() {
    echo ""
    echo -e "\${BLUE}================================\${NC}"
    echo -e "\${BLUE}  Uninstalling Kloudlite from Azure\${NC}"
    echo -e "\${BLUE}================================\${NC}"
    echo ""

    local uninstall_cmd="./kli azure uninstall --installation-key \$INSTALLATION_KEY"

    if [ -n "\$AZURE_LOCATION" ]; then
        uninstall_cmd="\$uninstall_cmd --location \$AZURE_LOCATION"
    fi

    if [ -n "\$AZURE_RESOURCE_GROUP" ]; then
        uninstall_cmd="\$uninstall_cmd --resource-group \$AZURE_RESOURCE_GROUP"
    fi
...
main() {
    echo -e "\${BLUE}================================\${NC}"
    echo -e "\${BLUE}  Kloudlite Azure Uninstaller\${NC}"
    echo -e "\${BLUE}================================\${NC}"
    echo ""
...
export async function GET(_request: NextRequest) {
  return new NextResponse(UNINSTALL_SCRIPT, {
    headers: {
      ...
      'Content-Disposition': 'inline; filename="uninstall-azure.sh"',
