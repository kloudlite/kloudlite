...
const tabs = [
  { id: 'access', label: 'Access Control', href: (id: string) => `/environments/${id}/settings/access` },
  { id: 'danger', label: 'Danger Zone', href: (id: string) => `/environments/${id}/settings/danger` },
]

export function EnvironmentSettingsTabs({ environmentId }: EnvironmentSettingsTabsProps) {
  const pathname = usePathname()
  const [underlineStyle, setUnderlineStyle] = useState({ left: 0, width: 0 })
  const tabRefs = useRef<Map<string, HTMLAnchorElement>>(new Map())

  const activeTab = tabs.find((tab) => pathname === tab.href(environmentId))?.id || 'access'
