    ...
    <div className="group relative border border-foreground/10 rounded-sm overflow-hidden bg-background">
      ...
      <div className="bg-foreground/[0.02] group-hover:bg-foreground/[0.03] border-b border-foreground/10 px-6 py-4 transition-colors duration-300 relative z-10">
        <div className="flex items-center gap-2 sm:gap-3 mb-2">
          ...
          <code className="text-foreground group-hover:text-primary font-mono text-sm sm:text-base font-semibold transition-colors duration-300">{title}</code>
          {alias && (
      ...
      <div className="p-4 sm:p-6 group-hover:bg-foreground/[0.015] transition-colors duration-300 relative z-10">
        {children}
      </div>
    </div>
  )
...
export function CodeExample({ title, children }: CodeExampleProps) {
  return (
    <div className="group">
      ...
      <div className="relative overflow-hidden bg-foreground/[0.04] hover:bg-foreground/[0.05] border border-foreground/10 hover:border-foreground/20 rounded-sm p-3 sm:p-4 font-mono text-[13px] overflow-x-auto transition-[background-color,border-color] duration-300">
        {/* Top accent line */}
        <div className="absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-transparent via-primary to-transparent scale-x-0 group-hover:scale-x-100 transition-transform duration-500 origin-center" />
