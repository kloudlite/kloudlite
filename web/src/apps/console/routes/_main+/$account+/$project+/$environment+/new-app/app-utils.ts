...
export const appInitialFormValues = ({
  ...
  getRepoName: ReturnType<typeof useAppState>['getRepoName'];
  getImageTag: ReturnType<typeof useAppState>['getImageTag'];
  getContainer: ReturnType<typeof useAppState>['getContainer'];
}) => {
  return {
    imageUrl: getContainer().image || '',
    ...
    repoName: getContainer().image ? getRepoName(getContainer().image) : '',

    repoImageTag: getContainer().image ? getImageTag(getContainer().image) : '',
    repoAccountName:
      ...
      containers: [
        {
          ...(s.spec.containers?.[0] || {}),
          image:
            val.repoAccountName === undefined || val.repoAccountName === ''
              ? `${val.repoName}:${val.repoImageTag}`
              : `${registryHost}/${val.repoAccountName}/${val.repoName}:${val.repoImageTag}`,
          name: 'container-0',
